#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""WitMotion IMU - Modbus-RTU driver (ROS 2)."""
import math
import time
...
class WitModbusNode(Node):
    ...
    def __init__(self):
        ...
        self.get_logger().info(
            'IMU Type: Modbus Port:%s Baud:%d' % (self.port, self.baud))
        find_ttyUSB()
    ...
    def poll_once(self):
        try:
            reg = self.master.execute(self.slave_id, cst.READ_HOLDING_REGISTERS, 52, 12)
        except Exception as e:
