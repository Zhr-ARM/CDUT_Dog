...
```bash
cd /home/zhr/DM_J4340_MOTOR
colcon build --packages-select dm_j4340_quad
...
```bash
ros2 launch dm_j4340_quad query_motor.launch.py
```

如果串口不是默认的 `/dev/ttyACM0`，可以覆盖参数：

```bash
ros2 launch dm_j4340_quad query_motor.launch.py serial_port:=/dev/ttyACM0
```
...
```bash
ros2 launch dm_j4340_quad start_motor.launch.py
```
...
- `config/query_motor.yaml`：电机扫描参数。
- `config/start_motor.yaml`：启动参数、MIT 话题、超时和量化范围。
- `launch/query_motor.launch.py`：启动 `query_motor_node`。
- `launch/start_motor.launch.py`：启动 `start_motor_node`。

`start_motor.yaml` 里的关键参数：
...
dm_j4340_quad/
  config/
    query_motor.yaml          电机扫描参数
    start_motor.yaml          MIT 启动和控制参数
  launch/
    query_motor.launch.py     启动 query_motor_node
    start_motor.launch.py     启动 start_motor_node
  src/dm_j4340_quad/
...
- `query_motor_node.py`：负责发送安全探测帧，并报告哪些电机 ID 在线。
- `start_motor_node.py`：负责启动序列、MIT 指令话题订阅、反馈缓存、周期发送和 ROS 服务。
