...
drives Gazebo through ros2_control by translating MIT commands into effort
commands for the joint effort controller.
"""
...
class SimMotorControllerNode(Node):
    def __init__(self) -> None:
        ...
        self.last_published_efforts = [0.0] * count
        ...
        self.command_timer = self.create_timer(
            1.0 / self.command_rate_hz, self._publish_effort_command
        )
        ...
        self.get_logger().info(
            ...
            f"{self.mit_command_topic} -> {self.effort_command_topic} "
    ...
    def _publish_effort_command(self) -> None:
        efforts = self._current_effort_command()
        self.last_published_efforts = list(efforts)

        msg = Float64MultiArray()
        msg.data = efforts
        self.effort_command_publisher.publish(msg)
    ...
    def _publish_status(self) -> None:
        ...
        efforts = ",".join(f"{value:.2f}" for value in self.last_published_efforts)

        msg = String()
        msg.data = (
            f"enabled={int(self.enabled)} mode={mode} "
            f"state_age={state_age:.3f} command_age={command_age:.3f} "
            f"positions=[{positions}] command_effort=[{efforts}] sim=1"
        )
